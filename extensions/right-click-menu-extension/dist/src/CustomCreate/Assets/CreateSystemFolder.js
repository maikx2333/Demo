...
async function createSystemFolder(dir) {
    let systemDir = dir + "/SystemFolders";
    let isExist = await Editor.Message.request("asset-db", "query-assets", { pattern: systemDir });
    if (isExist.length > 0) {
        return console.warn("自定义创建Assets 文件夹(系统),只允许在assets/resources/SystemFolders目录已存在!");
    }
    let res = await Editor.Message.request("asset-db", "create-asset", systemDir, null, { rename: true });
    if (res.isDirectory) {
